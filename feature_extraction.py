...
from pre_processing import filter_butter
...
def feature_extraction(data, sec=10, overlap_prosent=50):
    ...
    if overlap <=0:
        ...
        overlap =1
    cutoff = 6/20 
    data_high = filter_butter(data, cutoff, type='high')
    data_low = filter_butter(data, cutoff, type='low')
    ...
    for i in range(0, len(data)- time_steps, overlap):
        ...
        xs_low = data_low['x-axis'].values[i: i + time_steps]
        ys_low = data_low['y-axis'].values[i: i + time_steps]
        zs_low = data_low['z-axis'].values[i: i + time_steps]
       
        xs_high = data_high['x-axis'].values[i: i + time_steps]
        ys_high = data_high['y-axis'].values[i: i + time_steps]
        zs_high = data_high['z-axis'].values[i: i + time_steps]
        ...
        x_values_low.append(xs_low)
        y_values_low.append(ys_low)
        z_values_low.append(zs_low)
        x_values_high.append(xs_high)
        y_values_high.append(ys_high)
        z_values_high.append(zs_high)
        ...
        xy_corr.append(pearsonr(xs,ys)[0])
        xz_corr.append(pearsonr(xs,zs)[0])
        yz_corr.append(pearsonr(zs,ys)[0])
    ...
    # Energy: expect different energy for low and highpassed signal
    ...
    # feature_df['x_low_energy'] = pd.Series(x_values_low).apply(lambda x: np.sum(x**2)/100)
    # feature_df['y_low_energy'] = pd.Series(y_values_low).apply(lambda x: np.sum(x**2)/100)
    # feature_df['z_low_energy'] = pd.Series(z_values_low).apply(lambda x: np.sum(x**2/100))
    
    # feature_df['x_high_energy'] = pd.Series(x_values_high).apply(lambda x: np.sum(x**2)/100)
    # feature_df['y_high_energy'] = pd.Series(y_values_high).apply(lambda x: np.sum(x**2)/100)
    # feature_df['z_high_energy'] = pd.Series(z_values_high).apply(lambda x: np.sum(x**2/100))
    ...
    # FFT entropy:
    feature_df['x_entropi_fft'] = pd.Series(x_values_fft).apply(lambda x: entropy(x))
    feature_df['y_entropi_fft'] = pd.Series(y_values_fft).apply(lambda x: entropy(x))
    feature_df['z_entropi_fft'] = pd.Series(z_values_fft).apply(lambda x: entropy(x))
   ...
   # correlation:
    feature_df['xy_corr'] =pd.Series(xy_corr)
    feature_df['xz_corr'] = pd.Series(xz_corr)
    feature_df['yz_corr'] = pd.Series(yz_corr)
   ...
   #Capturing indices
    ...
    # correlation:
    # feature_df['xy_correlation'] = [pearsonr(x,y)[0] for x,y in map(x_values,y_values)]
